...
4. Verificar en la tarjeta: badge `−X% desc.` con el valor editado, desglose "Prima neta (sin desc.)" → "Descuento (X%)" → "Prima neta". Un descuento mayor debe bajar la prima total (posible "Mejor precio").
5. Click "Descargar PDF para el cliente" → se descarga `cotizacion-<cliente>.pdf` en `~/Downloads`. Abrir con `file:///home/ubuntu/Downloads/<archivo>.pdf` en Chrome y verificar título "Cotización de Seguro de Auto", datos del cliente/vehículo y tabla con columna "Descuento" consistente con la pantalla.
